...
//             obs_p_srv1_len1(1,2,2,i) = elem_prod(1.0-maturity_old_average(2),tmps);
    ...
    // for maturity and shell condition together in survey length comp fits
    {
        ...
        for (int i=1; i <= nobs_srv1_length; i++){
            ...
            offset(9) -= nsamples_srv1_length(MATURE,NEW_SHELL,sex,i)*vall*log(vall+p_const);
...
//    cout<<"writeReport"<<endl;
  ...
  report<<"----------------------------------------------------------------------------"<<endl;
  report<<endl;
  report<<f<<tb<<"#objective function value"<<endl;
  report<<"#idx   #weight      #likelihood      #objFun    #description"<<endl;
  for (int i=1;i<=NUM_FOUT;i++){
      report<<i<<tb<<wgtsOut(i)<<tb<<likeOut(i)<<tb<<objfOut(i)<<tb<<"#"<<strFOUT(i)<<endl;
  }
