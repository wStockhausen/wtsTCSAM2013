...
//     !! CheckFile<<catch_ghl<<endl;
 ...
 END_CALCS
    ...
    init_int phsTCF   //phase to turn on estimation of TCF fishing mortality on males (originally 1)
    init_int phsTCFF  //phase to turn on estimation for female offset to TCF fishing mortality
    init_int phsSCF   //phase to turn on estimation of SCF fishing mortality on males (originally 1)
    init_int phsSCFF  //phase to turn on estimation for female offset to SCF fishing mortality
    init_int phsRKF   //phase to turn on estimation of RKF fishing mortality on males (originally 1)
    init_int phsRKFF  //phase to turn on estimation for female offset to RKF fishing mortality
    init_int phsGTF   //phase to turn on estimation of GTF fishing mortality on males (originally 1)
    init_int phsGTFF  //phase to turn on estimation for female offset to GTF fishing mortality
    ...
    !!CheckFile<<"phsTCF  = "<<phsTCF<<endl;    
    !!CheckFile<<"phsTCFF = "<<phsTCFF<<endl;    
    !!CheckFile<<"phsSCF  = "<<phsTCF<<endl;    
    !!CheckFile<<"phsSCFF = "<<phsTCFF<<endl;    
    !!CheckFile<<"phsRKF  = "<<phsTCF<<endl;    
    !!CheckFile<<"phsRKFF = "<<phsTCFF<<endl;    
    !!CheckFile<<"phsGTF  = "<<phsTCF<<endl;    
    !!CheckFile<<"phsGTFF = "<<phsTCFF<<endl;    
...
//    init_bounded_dev_vector pFmDevsRKF(1,nobs_discardc_rkc,-15,15,phsFmRKF+1) // 
    ...
    init_number pAvgLnFmTCF(phsTCF)               //log-scale mean directed fishing mortality on males
    init_bounded_number pDltFmTCF(-2,2,phsTCFF) //log-scale offset to mean directed fishing mortality for females
    init_bounded_dev_vector pFmDevsTCF(1,nYrsDirectedFishery,-15,15,phsTCF+1) //log-scale directed fishing mortality devs IMPORTANT CHANGE: USED TO BE "1966,endyr-12"

    init_number pAvgLnFmSCF(phsSCF)                    // log-scale mean SCF fishing mortality on males
    init_bounded_number pDltFmSCF(-2,2,phsSCFF) //log-scale offset to mean SCF fishing mortality for females
    init_bounded_dev_vector pFmDevsSCF(1992,endyr-1,-15,15,phsSCF+1) // snow crab fishery f-devs   (IMPORTANT CHANGE: used to be "endyr")  1992 is OK

    init_bounded_number pAvgLnFmRKF(-10,5,phsRKF) // fishing mortality red king crab fishery discards //IMPORTANT CHANGE: now estimated
    init_bounded_number pDltFmRKF(-2,2,phsRKFF) //log-scale offset to mean RKF fishing mortality for females
    init_bounded_dev_vector pFmDevsRKF(1,nobs_discardc_rkc,-15,15,phsRKF+1) //this is NOT estimated (why?)  IMPORTANT CHANGE: now estimated!

    init_number pAvgLnFmGTF(phsGTF)              // log-scale mean groundfish fishing mortality on males
    init_bounded_number pDltFmGTF(-2,2,phsGTFF) //log-scale offset to mean GTF fishing mortality for females
    init_bounded_dev_vector pFmDevsGTF(1973,endyr-1,-15,15,phsGTF+1) // trawl fishery f-devs       (IMPORTANT CHANGE: used to be "endyr") 1973 seems OK
    ...
    matrix fmTCF_xy(1,nXs,styr,endyr-1)  // Fully-selected fishing mortality by sex in TCF 
    matrix fmSCF_xy(1,nXs,styr,endyr-1)  // Fully-selected fishing mortality by sex in SCF 
    matrix fmRKF_xy(1,nXs,styr,endyr-1)  // Fully-selected fishing mortality by sex in RKF 
    matrix fmGTF_xy(1,nXs,styr,endyr-1)  // Fully-selected fishing mortality by sex in GTF 
    number brSCF
    number brRKF
    ...
    4darray fTOT_xsyz(1,nXs,1,nSCs,styr,endyr-1,1,nlenm)  // total fishing mortality
...
//     exit(1);
    ...
    //for testing
//     CheckFile<<"fmTCF_xy       = "<<endl<<tb<<fmTCF_xy<<endl;
//     CheckFile<<"fmSCF_xy = "<<endl<<tb<<fmSCF_xy<<endl;
//     CheckFile<<"fmRKF_xy   = "<<endl<<tb<<fmRKF_xy<<endl;
//     CheckFile<<"fmGTF_xy      = "<<endl<<tb<<fmGTF_xy<<endl;
...
FUNCTION void jitterParameters(double fac)   //wts: new 2014-05-10
    ...
    wts::jitterParameter(pDltFmTCF,fac,rng);  // female offset for log-scale mean directed fishing mortality
    ...
    wts::jitterParameter(pDltFmSCF,fac,rng);  // female offset for log-scale snow crab fishing mortality
    wts::jitterParameter(pFmDevsSCF,0.1*fac,rng);// snow crab fishery f-devs   (IMPORTANT CHANGE: used to be "endyr")  1992 is OK
    wts::jitterParameter(pAvgLnFmRKF,fac,rng);   // fishing mortality red king crab fishery discards
    wts::jitterParameter(pDltFmRKF,fac,rng);  // female offset for log-scale red king crab fishing mortality
    wts::jitterParameter(pFmDevsRKF,0.1*fac,rng);// rkf f-devs
    wts::jitterParameter(pAvgLnFmGTF,fac,rng);   // log-scale mean groundfish fishing mortality
    wts::jitterParameter(pDltFmGTF,fac,rng);  // female offset for log-scale groundfish fishing mortality
    wts::jitterParameter(pFmDevsGTF,0.1*fac,rng);// groundfish fishery f-devs       (IMPORTANT CHANGE: used to be "endyr") 1973 seems OK
...
FUNCTION get_mortality
    ...
    fmTCF_xy.initialize();
    fmTCF_xy(  MALE)(styr,1964) = 0.05;//was 1965!!
    fmTCF_xy(FEMALE)(styr,1964) = 0.05*mfexp(pDltFmTCF);//was 1965!!
//    cout<<"0a"<<endl;
    int idx = 1;
    for(int iy =1965;iy<endyr;iy++){
        if(hasDirectedFishery(iy)) {
            fmTCF_xy(  MALE,iy) = mfexp(pAvgLnFmTCF+pFmDevsTCF(idx++));
            fmTCF_xy(FEMALE,iy) = mfexp(pAvgLnFmTCF+pDltFmTCF+pFmDevsTCF(idx++));
        }
    }
    ...
    //  cout<<" brSCF "<<endl; 
    fmSCF_xy.initialize();
    fmSCF_xy(  MALE)(styr,1977)= 0.01;
    fmSCF_xy(FEMALE)(styr,1977)= 0.01*mfexp(pDltFmSCF);
    fmSCF_xy(  MALE)(1978,1991) = brSCF*effSCF(1978,1991);
    fmSCF_xy(FEMALE)(1978,1991) = brSCF*effSCF(1978,1991)*mfexp(pDltFmSCF);
    fmSCF_xy(  MALE)(1992,endyr-1) = fmSCF1;
    fmSCF_xy(FEMALE)(1992,endyr-1) = fmSCF1*mfexp(pDltFmSCF);
    if (debug) cout<<"3"<<endl;
    
    // need to have the devs 1992 to present
    dvar_vector fcRKF1 = mfexp(pAvgLnFmRKF+pFmDevsRKF);
    brRKF = mean(1-exp(-fcRKF1))/(mean(effRKF(yrs_discardc_rkc)));
    fmRKF_xy.initialize();
    fmRKF_xy(  MALE)(styr,1952)= 0.02;
    fmRKF_xy(  MALE)(styr,1952)= 0.02*mfexp(pDltFmRKF);
    fmRKF_xy(  MALE)(1953,1991) = -log(1-brRKF*effRKF(1953,1991));
    fmRKF_xy(FEMALE)(1953,1991) = -log(1-brRKF*effRKF(1953,1991))*mfexp(pDltFmRKF);
    for (int iy=1;iy<=nobs_discardc_rkc;iy++) {
        int y = yrs_discardc_rkc(iy);
        fmRKF_xy(  MALE)(y) = mfexp(pAvgLnFmRKF+pFmDevsRKF(iy));
        fmRKF_xy(FEMALE)(y) = mfexp(pAvgLnFmRKF+pFmDevsRKF(iy)+pDltFmRKF);
    }
    fmRKF_xy(  MALE)(1984,1985) = 0.0;
    fmRKF_xy(FEMALE)(1984,1985) = 0.0;
    fmRKF_xy(  MALE)(1994,1995) = 0.0;
    fmRKF_xy(FEMALE)(1994,1995) = 0.0;
    if (debug) cout<<"4"<<endl;
   
    fmGTF_xy.initialize();
    fmGTF_xy(  MALE)(styr,1972) = mean(mfexp(pAvgLnFmGTF+pFmDevsGTF)); 
    fmGTF_xy(FEMALE)(styr,1972) = mean(mfexp(pAvgLnFmGTF+pFmDevsGTF+pDltFmGTF));
    fmGTF_xy(  MALE)(1973,endyr-1) = mfexp(pAvgLnFmGTF+pFmDevsGTF(1973,endyr-1));
    fmGTF_xy(FEMALE)(1973,endyr-1) = mfexp(pAvgLnFmGTF+pFmDevsGTF(1973,endyr-1)+pDltFmGTF);
    ...
    for (int iy=styr;iy<endyr;iy++) {
        ...
        fTCFF_yz(iy)= selTCFF*fmTCF_xy(FEMALE,iy);
        ...
        fGTF_xyz(FEMALE,iy) = sel_trawl_use(FEMALE)    *fmGTF_xy(FEMALE,iy);   
        fGTF_xyz(  MALE,iy) = sel_trawl_use(  MALE)    *fmGTF_xy(  MALE,iy);   
        fSCF_xyz(FEMALE,iy) = sel_disc_snow_use(FEMALE)*fmSCF_xy(FEMALE,iy);   
        fSCF_xyz(  MALE,iy) = sel_disc_snow_use(  MALE)*fmSCF_xy(  MALE,iy);   
        fRKF_xyz(FEMALE,iy) = sel_disc_rkc_use(FEMALE) *fmRKF_xy(FEMALE,iy);   
        fRKF_xyz(  MALE,iy) = sel_disc_rkc_use(  MALE) *fmRKF_xy(  MALE,iy);   
        
        for(int s=NEW_SHELL;s<=OLD_SHELL;s++) { //over new (shell=1) and old (shell=2) shell...
            fTCFM_syz(s,iy)     = selTCFM(s,iy)*fmTCF_xy(MALE,iy);       
            fTCFR_syz(s,iy)     = selTCFR(s,iy)*fmTCF_xy(MALE,iy);
            fTOT_xsyz(FEMALE,s,iy) = fTCFF_yz(iy)   + fGTF_xyz(FEMALE,iy)+fSCF_xyz(FEMALE,iy)+fRKF_xyz(FEMALE,iy);
            fTOT_xsyz(  MALE,s,iy) = fTCFM_syz(s,iy)+ fGTF_xyz(  MALE,iy)+fSCF_xyz(  MALE,iy)+fRKF_xyz(  MALE,iy);
            S_xsyz(FEMALE,s,iy) = mfexp(-1.0*fTOT_xsyz(FEMALE,s,iy));//wts: does not depend on s
            S_xsyz(  MALE,s,iy) = mfexp(-1.0*fTOT_xsyz(  MALE,s,iy));
        }//shell category
    }//year
...
//  report << bm2 << endl;
  ...
  report << "observed redk female discard mortality biomass: seq(1992,"<<endyr-1<<")" << endl;
   for (i=1; i<=nobs_discardc_rkc; i++) report << catch_rkodisc(1)(i) <<" ";
  report << endl;
    //the following were 'seq("<<styr<<","<<endyr<<")"'
  ...
  report << "observed redk male discard mortality biomass: seq(1992,"<<endyr-1<<")" << endl;
   for (i=1; i<=nobs_discardc_rkc; i++) report << catch_rkodisc(2)(i) <<" ";
  report << endl;
  ...
  report << "estimated male annual directed fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmTCF_xy(  MALE)(styr,endyr-1) << endl;
  report << "estimated female annual directed fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmTCF_xy(FEMALE)(styr,endyr-1) <<endl;
  report << "estimated male annual snow fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmSCF_xy(  MALE)(styr,endyr-1) << endl;
  report << "estimated female annual snow fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmSCF_xy(FEMALE)(styr,endyr-1) << endl;
  report << "estimated male annual red king fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmRKF_xy(  MALE)(styr,endyr-1) << endl;
  report << "estimated female annual red king fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmRKF_xy(FEMALE)(styr,endyr-1) << endl;
  report << "estimated male annual groundfish fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmGTF_xy(  MALE)(styr,endyr-1) <<endl;
  report << "estimated female annual groundfish fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  report << fmGTF_xy(FEMALE)(styr,endyr-1) <<endl;
  
  report <<"max retained fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fTCFR_syz(1,i))<<" "; report<<endl;  
  report <<"max male annual directed fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fTCFM_syz(1,i))<<" "; report<<endl;  
  report <<"max female annual directed fishing mortality rate: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fTCFF_yz(i))<<" "; report<<endl;  
  report << "max male annual snow fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fSCF_xyz(  MALE,i))<<" "; report<<endl;  
  report << "max female annual snow fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fSCF_xyz(FEMALE,i))<<" "; report<<endl;  
  report << "max male annual red king fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fRKF_xyz(  MALE,i))<<" "; report<<endl;  
  report << "max female annual red king fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fRKF_xyz(FEMALE,i))<<" "; report<<endl;  
  report << "max male annual groundfish fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fGTF_xyz(  MALE,i))<<" "; report<<endl;  
  report << "max female annual groundfish fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report <<max(fGTF_xyz(FEMALE,i))<<" "; report<<endl;  
  report << "max male annual total fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report << max(fTOT_xsyz(  MALE,1,i)) <<" "; report<< endl;
  report << "max female annual total fishing mortality: seq("<<styr<<","<<endyr-1<<")" << endl;
  for(i=styr;i<endyr;i++) report << max(fTOT_xsyz(FEMALE,1,i)) <<" "; report<< endl;
...
//  report << selTCFR(1,1991) << endl;
  ...
  for(i=1991;i<=1996; i++) 
   { for(j=1; j<=nlenm; j++){report <<retFcn(1,i,j)<<" ";
   }
      report<<endl;
   }
  //wts 2014-04-25 changed from selTCFR to retFcn
...
//  report << sel_srv1(2) << endl;
  ...
  report<<len_len<<endl;
  report<<"#male directed fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmTCF_xy(MALE)(endyr-5,endyr-1))<<endl;
  report<<"#female directed fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmTCF_xy(FEMALE)(endyr-5,endyr-1))<<endl;
  report<<"#male trawl fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmGTF_xy(  MALE)(endyr-5,endyr-1))<<endl;
  report<<"#female trawl fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmGTF_xy(FEMALE)(endyr-5,endyr-1))<<endl;
  report<<"#male snow fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmSCF_xy(  MALE)(endyr-5,endyr-1))<<endl;
  report<<"#female snow fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmSCF_xy(FEMALE)(endyr-5,endyr-1))<<endl;
  report<<"#male red king fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmRKF_xy(  MALE)(endyr-5,endyr-1))<<endl;
  report<<"#female red king fishing mortality average last 5 yrs"<<endl;
  report<<mean(fmRKF_xy(FEMALE)(endyr-5,endyr-1))<<endl;
...
//    cout<<"starting writeToR"<<endl;
  // stuff for input to projection model
      ...
      os<<0<<tb<<tb<<"#inpFmTCF: input F for male directed Tanner crab fishing mortality"<<endl;
      os<<pDltFmTCF<<tb<<tb<<"#inpDltFmTCF: input log-scale female offset for directed Tanner crab fishing mortality"<<endl;
      os<<mean(fmSCF_xy(  MALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmSCF: input   male F for snow crab fishing mortality rate"<<endl;
      os<<mean(fmSCF_xy(FEMALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmSCF: input female F for snow crab fishing mortality rate"<<endl;
      os<<mean(fmRKF_xy(  MALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmRKF: input   male F for BBRKC  fishing mortality rate"<<endl;
      os<<mean(fmRKF_xy(FEMALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmRKF: input female F for BBRKC  fishing mortality rate"<<endl;
      os<<mean(fmGTF_xy(  MALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmGTF: input   male F for groundfish fishery fishing mortality rate"<<endl;
      os<<mean(fmGTF_xy(FEMALE)(endyr-5,endyr-1))<<tb<<tb<<"#inpFmGTF: input female F for groundfish fishery fishing mortality rate"<<endl;
...
//    cout<<"starting writeToR"<<endl;
        ...
        R_out <<"$retained.fmTCFR_syz" << endl;
        for (int i=styr;i<=(endyr-1);i++) R_out <<max(fTCFR_syz(1,i))<<" "; R_out<<endl;
        R_out << "$estimated.annual.total.directed.male.fishing.mortality" << endl;
        R_out << fmTCF_xy( MALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.total.directed.female.fishing.mortality" << endl;
        R_out << fmTCF_xy(FEMALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.snow.male.fishing.mortality" << endl;
        R_out << fmSCF_xy( MALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.snow.female.fishing.mortality" << endl;
        R_out << fmSCF_xy(FEMALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.red.king.male.fishing.mortality" << endl;
        R_out << fmRKF_xy( MALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.red.king.female.fishing.mortality" << endl;
        R_out << fmRKF_xy(FEMALE)(styr,endyr-1) << endl;
        R_out << "$estimated.annual.trawl.male.fishing.mortality" << endl;
        R_out << fmGTF_xy(  MALE)(styr,endyr-1) <<endl;
        R_out << "$estimated.annual.trawl.female.fishing.mortality" << endl;
        R_out << fmGTF_xy(FEMALE)(styr,endyr-1) <<endl;
        R_out << "$estimated.annual.total.fishing.mortality.males" << endl;
        for (int i=styr;i<=(endyr-1);i++) R_out << max(fTOT_xsyz(MALE,1,i)) <<" "; R_out<< endl;
        R_out << "$estimated.annual.total.fishing.mortality.females" << endl;
        for (int i=styr;i<=(endyr-1);i++) R_out << max(fTOT_xsyz(FEMALE,1,i)) <<" "; R_out<< endl;
