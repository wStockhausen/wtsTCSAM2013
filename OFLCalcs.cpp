...
PopDyInfo::PopDyInfo(int npZBs, double dtMp){
    ...
    nMSs = nMATURITY_STATES;
    nSCs = nSHELL_CONDITIONS;
...
double OFL_Calculator::calcFofl(double Bmsy, double Fmsy, d3_array& n_msz, ostream& cout){
    if (debug) {
        ...
        cout<<"n_msz = "<<endl;
        for (int m=1;m<=nMATURITY_STATES;m++) {cout<<"m = "<<m<<endl; cout<<n_msz(m)<<endl;}
    }
...
double OFL_Calculator::calcMSY(dvector R_x, double Fmsy, ostream& cout){
    ...
    msy_fx.allocate(0,nFsh,1,nSEXS);
    msy_fx.initialize();
...
double OFL_Calculator::calcOFL(double Fofl, d4_array& n_xmsz, ostream& cout){
    ...
    ofl_fx.allocate(0,nFsh,1,nSEXS);
    ofl_fx.initialize();
...
OFLResults OFL_Calculator::calcOFLResults(dvector R, d4_array& n_xmsz, ostream& cout){
    ...
    res.ofl_fx.allocate(0,nFsh,1,nSEXS);
    res.ofl_fx = ofl_fx;
...
void OFLResults::writeToCSV(ostream& os){
    os<<avgRec_x(  MALE);
    if (nSEXS>1) os<<cc<<avgRec_x(FEMALE);
...
void OFLResults::writeToR(ostream& os, adstring name, int debug){
    ...
    if (nSEXS>1) os<<cc<<"avgRecF="<<avgRec_x(FEMALE);
