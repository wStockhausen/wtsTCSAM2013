...
//     !! CheckFile<<catch_ghl<<endl;
 ...
 END_CALCS
    ...
    //---------------------------------------------------------------------------------------
    // Open control file....
    !! ad_comm::change_datafile_name("TCSAM_ControlFile.dat");
    !!CheckFile<<"Reading control file 'TCSAM_ControlFile.dat'"<<endl;
...
PARAMETER_SECTION
    ...
    init_bounded_vector mat_big(1,nXs,0.1,10.0,8)               // mult. on 1980 M_imm for mature males and females                     
    ...
    vector rec_y(styr,endyr)                                 //arithmetic-scale recruitments
...
//     CheckFile<<"catch_disc(2) "<<catch_disc(2)<<endl;
    ...
    for (int i=1;i<=nobs_trawl;i++) {
        ...
        for(int sex=1;sex<=nXs;sex++) offset(5) -= nsamples_trawl(sex,i)*obs_p_trawl(sex,i)*log(p_const+obs_p_trawl(sex,i));
    }
    ...
    for (int i=1;i<=nobs_snowfish_discf;i++) {
        ...
        offset(6) -= nsamples_snowfish_discf(i)*(obs_p_snow(FEMALE,i)*log(p_const+obs_p_snow(FEMALE,i)));
    }
    ...
    for (int i=1;i<=nobs_snowfish_discm;i++) {
        ...
        offset(6) -= nsamples_snowfish_discm(1,i)*obs_p_snow(MALE,i)*log(p_const+obs_p_snow(MALE,i));
    }
    ...
    for (int i=1;i<=nobs_rkfish_discm;i++) {
        ...
        offset(8)-=nsamples_rkfish_discm(NEW_SHELL,i)*obs_p_rk(MALE,i)*log(p_const+obs_p_rk(MALE,i));
    }
        ...
        for (int mat=1;mat<=nMSs;mat++)
            for (int shell=1; shell<=nSCs; shell++)
                for (int sex=1; sex<=nXs; sex++)
                    sumsrv(ll) += sum(obs_p_srv1_lend(mat,shell,sex,ll));
            ...
            for (int sex=1; sex<=nXs;sex++) {                //sex
                for (int i=1; i <= nobs_srv1_length; i++){           //year
                    if(mat==IMMATURE){
                        ...
                        if( shell==NEW_SHELL){
                            obs_p_srv1_len1(mat,shell,sex,i)=fraction_new_error*obs_p_srv1_lend(mat,shell,sex,i)/sumsrv(i);
                        } else {
...
//             obs_p_srv1_len1(1,2,2,i) = elem_prod(1.0-maturity_old_average(2),tmps);
    ...
    for(int sex=1; sex<=nXs;sex++){ //sex
        for (int i=1; i <= nobs_srv1_length; i++){
            ...
            offset(4) -= nsamples_srv1_length(MATURE,NEW_SHELL,sex,i)*vall*log(vall+p_const);
            dvector val2 = obs_p_srv1_len(  MATURE,NEW_SHELL,sex,i)+obs_p_srv1_len(  MATURE,OLD_SHELL,sex,i);
            offset(4) -= nsamples_srv1_length(MATURE,OLD_SHELL,sex,i)*val2*log(val2+p_const);
        }
    }
    CheckFile<<"offset(4) = "<<offset(4)<< endl;  
...
//    writeToR(R_out);             //for testing
    ...
    if((call_no==1)||(call_no/100)*100==call_no) {
        CheckFile<<"call_no = "<<call_no<<endl;
        writeParameters(CheckFile);
        CheckFile <<"Likes = "<< Fout << endl;
        CheckFile <<"phase = "<< current_phase() << " call = " << call_no << " Total Like = " << f << endl;
        writeReport(CheckFile); exit(1);
    }    
//     //for testing
...
//     cout <<"phase = "<< current_phase() << " call = " << call_no << " Total Like = " << f << endl;
    ...
    if (mceval_phase()) myWriteMCMCToR(mcmc);
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameters(ofstream& os)                        //wts: new
    os<<"pMnLnRec      = "<<pMnLnRec<<endl;
    os<<"pRecDevs      = "<<pRecDevs<<endl;
    os<<"pMnLnRecEarly = "<<pMnLnRecEarly<<endl;
    os<<"pRecDevsEarly = "<<pRecDevsEarly<<endl;
    os<<"pAvgLnFmTCF   = "<<pAvgLnFmTCF<<endl;
    os<<"pFmDevsTCF    = "<<pFmDevsTCF<<endl;
    
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameterBounds(ofstream& os, param_init_number& p)                        //wts: new
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameterBounds(ofstream& os, param_init_bounded_number& p)                        //wts: new
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameterBounds(ofstream& os, param_init_vector& p)                        //wts: new
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameterBounds(ofstream& os, param_init_bounded_vector& p)                        //wts: new
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
FUNCTION void writeParameterBounds(ofstream& os, param_init_bounded_dev_vector& p)                        //wts: new
...
//         Fout(2) = penal_sexr; f += penal_sexr;
    ...
    if(active(srv3_q) && q_prior_switch>0) {  
        ...
        f += srv3q_penalty; Fout(4) = srv3q_penalty;
    }
    ...
    if(active(matestm)) {
        ...
        like_mat += 0.5*norm2(first_difference(first_difference(matestm)));//wts: why different weights? 1.0 vs. 0.5?
        f += like_mat;
        Fout(3)= like_mat;
    }
    ...
    if(active(log_sel50_dev_mn)) sel_50m_penal = like_wght_sel50*norm2(first_difference(log_sel50_dev_mn));
    ...
    if (active(pFmDevsTCF)) {        
        fpen += 1.*norm2(pFmDevsTCF);                               //wts: need to turn this off in last phase?
    }
    if(active(pFmDevsSCF)) fpen += 0.5*norm2(pFmDevsSCF);//wts: need to turn this off in last phase? note that relative weights are hard-wired
    if(active(pFmDevsRKF)) fpen += 3.0*norm2(pFmDevsRKF);//wts: need to turn this off in last phase?
    if(active(pFmDevsGTF)) fpen += 0.5*norm2(pFmDevsGTF);//wts: need to turn this off in last phase?
...
//    cout<<"4"<<endl;    
    ...
    for (int i=1; i <= nobs_snowfish_discm; i++) {
        ...
        len_like(6) -= nsamples_snowfish_discm(NEW_SHELL,i)*(obs_p_snow(MALE,i)*log(pred_p_snow(MALE,yr)+p_const));
    }
    CheckFile<<"+++++++++++++++++"<<endl;
    CheckFile<<"snf males LL = "<<len_like(6)<<endl;
...
//    cout<<" snow m length "<<endl;
    for (int i=1; i <= nobs_snowfish_discf; i++) {
        ...
        len_like(6) -= nsamples_snowfish_discf(i)*(obs_p_snow(FEMALE,i)*log(pred_p_snow(FEMALE,yr)+p_const));
    }
//    cout<<"1"<<endl;    
    ...
    for (int i=1; i <= nobs_rkfish_discf; i++) {
        ...
        len_like(8) -= nsamples_rkfish_discm(NEW_SHELL,i)*(obs_p_rk(MALE,i)*log(pred_p_rk(MALE,yr)+p_const));
    }
//    cout<<" red f length "<<endl;
    CheckFile<<"+++++++++++++++++"<<endl;
    CheckFile<<"rkf males LL = "<<len_like(8)<<endl;
    ...
    for (int i=1; i <= nobs_trawl; i++) {
        yr=yrs_trawl(i);
        for(int sex=1;sex<=nXs;sex++){
            len_like(5) -= nsamples_trawl(sex,i)*(obs_p_trawl(sex,i)*log(pred_p_trawl(sex,yr)+p_const));
        }
    }
//    cout<<" trawl length "<<endl;
    ...
    for(int sex=1;sex<=2;sex++){
        for (int i=1; i <=nobs_srv1_length; i++) {
            ...
            len_like(4) -= nsamples_srv1_length(MATURE,NEW_SHELL,sex,i)*(
            ...
            len_like(4) -= nsamples_srv1_length(IMMATURE,NEW_SHELL,sex,i)*(
...
//    cout<<"1"<<endl;    
    ...
    nextf = like_wght(1)*len_like(1);         // Retained fishery 
    Fout(12) = nextf; f += nextf;
    
    nextf = like_wght(2)*len_like(2);         // Total (ret+disc)
    Fout(13) = nextf; f += nextf;
    
    nextf = like_wght(3)*len_like(3); Fout(14) = nextf; f+= nextf;    // Female
    nextf = like_wght(4)*len_like(4); Fout(15) = nextf; f+= nextf;    // Survey
    nextf = like_wght(7)*len_like(5); Fout(16) = nextf; f+= nextf;    // Trawl
    //take out length likelihoods for bycatch for testing
    
    nextf = len_like(6); Fout(17) = nextf; f+= nextf;    // snow fishery
    
    //     nextf = (len_like(7)); Fout(18) = nextf; f+= nextf;    // red king fishery male only
    nextf = (len_like(7)+len_like(8)); Fout(18) = nextf; f+= nextf;    // red king fishery male plus female
    ...
    for(int i=1;i<=nobs_srv1;i++) {
        ...
        cv_srv1(FEMALE,yrs_srv1(i))  = like_wght(5)  *cv_srv1o(FEMALE,i);  
...
//    cout<<"2"<<endl;    
    ...
    Fout(19) = surv_like;  f += surv_like; 
    
    // Male tot catch
    catch_like1.initialize();
    catch_like1 = norm2(obs_catchtot_biom(yrs_fish_catchf)-pred_catch(yrs_fish_catchf));
    nextf = like_wght(6)*catch_like1; Fout(21) = nextf; f += nextf;
    // Male retained catch
    catch_like2.initialize();
    catch_like2 = norm2((catch_ret(1965,endyr-1))-(pred_catch_ret(1965,endyr-1)));
    nextf = like_wght(6)*catch_like2; Fout(22) = nextf; f += nextf;
    ...
    nextf = like_wght(6)*catch_likef; Fout(23) = nextf; f+= nextf;
    //trawl fishery
    catch_liket.initialize();
    catch_liket = norm2((obs_catcht_biom(yrs_trawl_c))-(pred_catch_trawl(yrs_trawl_c)));
    nextf = like_wght(6)*catch_liket;  Fout(24) = nextf; f += nextf;
    //  snow crab + rkc fisheries
    for(int i=1;i<=nobs_discardc_snow;i++){
        pred_tmp_snow(1,i)=pred_catch_female_snowd(yrs_discardc_snow(i)); //females
        pred_tmp_snow(2,i)=pred_catch_snowd(yrs_discardc_snow(i));        //males
    }
//    cout<<"3"<<endl;    
...
//    cout<<"4"<<endl;    
    ...
    nextf = like_wght(6)*catch_likes; Fout(25) = nextf; f += nextf;
    //  rkc fishery
    ...
    nextf = like_wght(6)*catch_liker; Fout(26) = nextf; f += nextf;
//    cout<<" red disc "<<endl;
    // ------------------------------------------
    //  }
    call_no += 1;
...
// in the model spring 1979 is stil 1978.  the last recruits are 2003 that come in spring 2004
  report << "estimated number of recruitments female: seq("<<styr<<","<<endyr<<")" << endl;      //IMPORTANT CHANGE: was 'seq("<<styr+1<<","<<endyr<<")"'
  report << rec_y <<endl;//IMPORTANT CHANGE: was i<=(endyr-1)
  report <<endl<< "estimated number of recruitments male: seq("<<styr<<","<<endyr<<")" << endl;//IMPORTANT CHANGE: was 'seq("<<styr+1<<","<<endyr<<")"'
  report << rec_y <<endl;//IMPORTANT CHANGE: was i<=(endyr-1)
...
//  report << sel_srv1(2) << endl;
  ...
  report <<"likelihood weights:  'rec devs','sex ratio','fishery 50%','fmTCF_y phase 1','fmTCF_y phase>1','fmTCF_y devs'"<<endl;
  ...
  report <<"#recruitments female, male start year to endyr from model" << endl;//was endyr-1
  report << rec_y <<endl;//was endyr-1
  report << "#recruitments male, male start 1960 to endyr from model" << endl;//was endyr-1
  report << rec_y <<endl;//was endyr-1
...
//    cout<<"starting writeToR"<<endl;
  // stuff for input to projection model
      ...
      os<<"#--------------------------------------------------------------------"<<endl;
      os<<endyr<<tb<<tb<<"#start year for projection model (model end year)"<<endl;
      os<<0    <<tb<<tb<<"#number of years for projections"<<endl;
      os<<0    <<tb<<tb<<"#number of projections to make"<<endl;
      ...
      os<<mean(rec_y(1982,endyr))<<tb<<mean(rec_y(1982,endyr))<<tb<<"#average recruitment by sex for BXX/Bmsy calculation"<<endl;
      ...
      os<<"#SRR info"<<endl;
      os<<"#---------------------------"<<endl;
      os<<"???" <<tb<<tb<<"#srType  : flag indicating type of SRR"<<endl;
      os<<"???" <<tb<<tb<<"#recGamma: recruitment autocorrelation parameter"<<endl;
      ...
      os<<"#---------------------------"<<endl;
      os<<endl;//was endyr-1
      os<<endyr-styr+1<<tb<<tb<<"#nYrsAsmtMod: number of recruits from the model styr to endyr"<<endl;
      os<<endyr-1982+1<<tb<<tb<<"#nYrsAvgRec : number of recruits for avg to estimate B35%"<<endl;
      os <<"#asmtModRec(nXs,nYrsAsmtMod): unlagged recruitments female, male start year to endyr from model" << endl;//was endyr-1
      os << rec_y <<endl;//was endyr-1
      os << rec_y <<endl;//was endyr-1     
      os<<"#asmtModSpB(1,nYrsAsmtMod: male spawning biomass at matetime for str year to endyr-1 for spawner recruit curve to estimate recruitments"<<endl;
      os<<mspbio_matetime(styr,endyr-1)<<endl;
      os<<"#---------------------------"<<endl;
      os<<"#Pop info in last year of assessment model"<<endl;
      os<<"#---------------------------"<<endl;
      os<<"#numbers at size immature new shell female, male in final year"<<endl;
      os<<natlength_inew(FEMALE,endyr)<<endl;
      os<<natlength_inew(  MALE,endyr)<<endl;
      os<<"#numbers at length immature old shell female male last year"<<endl;
      os<<natlength_iold(FEMALE,endyr)<<endl;
      os<<natlength_iold(  MALE,endyr)<<endl;
      os<<"#numbers at length mature new shell female male last year"<<endl;
      os<<natlength_mnew(FEMALE,endyr)<<endl;
      os<<natlength_mnew(  MALE,endyr)<<endl;
      os<<"#numbers at length mature old shell female male last year"<<endl;
      ...
      os<<mean(fmSCF_y(endyr-5,endyr-1))<<tb<<tb<<"#inpFmSCF: input F for directed Tanner crab fishing mortality"<<endl;
      ...
      os<<(selTCFM(1,endyr-4)+selTCFM(1,endyr-3)+selTCFM(1,endyr-2)+selTCFM(1,endyr-1))/4.0<<endl;
      os<<(selTCFM(1,endyr-4)+selTCFM(2,endyr-3)+selTCFM(2,endyr-2)+selTCFM(2,endyr-1))/4.0<<endl;
      os<<"#selTCF_RetMale(nSCs,nXs): average of last 4 years selTCFM retained curve male new old shell"<<endl;
      os<<(selTCFR(1,endyr-4)+selTCFR(1,endyr-3)+selTCFR(1,endyr-2)+selTCFR(1,endyr-1))/4.0<<endl;//IMPORTANT CHANGE: was only over last 3 years (but said 4)
      os<<(selTCFR(2,endyr-4)+selTCFR(2,endyr-3)+selTCFR(2,endyr-2)+selTCFR(2,endyr-1))/4.0<<endl;
      ...
      os<<(selTCFM(1,endyr-4)+selTCFM(1,endyr-3)+selTCFM(1,endyr-2)+selTCFM(1,endyr-1))/4.0<<endl;
      os<<(selTCFM(1,endyr-4)+selTCFM(2,endyr-3)+selTCFM(2,endyr-2)+selTCFM(2,endyr-1))/4.0<<endl;
      os<<"#selTCF_RetMaleWest(nSCs,nXs): set same as avg retained, but shifted to lower end"<<endl;
      os<<(selTCFR(1,endyr-4)+selTCFR(1,endyr-3)+selTCFR(1,endyr-2)+selTCFR(1,endyr-1))/4.0<<endl;//IMPORTANT CHANGE: was only over last 3 years (but said 4)
      os<<(selTCFR(2,endyr-4)+selTCFR(2,endyr-3)+selTCFR(2,endyr-2)+selTCFR(2,endyr-1))/4.0<<endl;
      ...
      os<<"#weight at length female juvenile"<<endl;
      os<<wtf(IMMATURE)<<endl;
      os<<"#weight at length female mature"<<endl;
      os<<wtf(MATURE)<<endl;
      os<<"#weight at length male"<<endl;
      os<<wtm<<endl;
...
//         R_out << "$estimated.annual.fishing.mortality.females.pot" << endl;
        ...
        //  R_out <<fish_sel50_mn(1981,endyr-1)<<endl;
...
REPORT_SECTION
    ...
    ofstream osR("TCSAM_WTS."+str(current_phase())+".R", ios::trunc);    
    myWriteToR(osR);
    osR.close();
    ...
    if (last_phase()) {
        R_out.open("TCSAM_WTS.oldstyle.R");
        writeToR(R_out);
        R_out.close();
        
        R_out.open("TCProjMod2013.dat");
        writeMyProjectionFile(R_out);
        R_out.close();
    }
//  report << offset << endl;
...
TOP_OF_MAIN_SECTION
  ...
  gradient_structure::set_NUM_DEPENDENT_VARIABLES(600);
  time(&start);
